...
export class GeoJSONLayer extends Layer {
  ...
  applyLevelFilter() {
    ...
    renderDescBanners(this.layer.geojson?.features || [], this.id, this.show !== false)
  }
  ...
  loadData() {
    ...
    return new Promise((resolve) => {
      ...
      const onData = (e) => {
        ...
        map.off('sourcedata', onData)
        // An event of the empty initial source can land here before the url is loaded. getData()
        // then waits for that load, so keep onError: a failed fetch would never resolve us.
        source.getData()
      ...
      // A failed url fetch fires 'error' instead of 'sourcedata'. MapLibre keeps the old features
      // on screen, so we keep this.layer.geojson as well, or counts and lookups go out of sync.
      // Resolve rather than reject, so one failing layer doesn't hang the Promise.all of the caller.
      const onError = (e) => {
  ...
  render(resetDraw = true, { sourceLoaded = false } = {}) {
    ...
    renderDescBanners(features, this.id, this.show !== false)
  ...
  applyFeatureUpdate(feature, { resetDraw = false, refreshRouteExtras = false, refreshKmMarkers = false } = {}) {
    ...
    syncDescBanner(feature, this.id, this.show !== false)
  ...
  applyFeatureRemove(feature) {
    ...
    removeDescBanner(feature, this.id)
