...
export function initSettingsModal () {
  functions.e('#settings-modal', e => {
    ...
    e.setAttribute('data-map--settings-default-pitch-value', Math.round(mapProperties.pitch || map?.getPitch() || 0))
    if (mapProperties.zoom) {
      e.setAttribute('data-map--settings-default-zoom-value', parseFloat(mapProperties.zoom).toFixed(2))
    } else {
      e.removeAttribute('data-map--settings-default-zoom-value')
    }
